...
class WxPayDataBase
{
	...
	**/
	 ...
	 */
	public function MakeSign($WxCfg)
	{
		//签名步骤一：按字典序排序参数
		ksort($this->values);
		$string = $this->ToUrlParams();
		//签名步骤二：在string后加入KEY
		$string = $string . "&key=".$WxCfg->getKEY();
		//签名步骤三：MD5加密
		$string = md5($string);
		//签名步骤四：所有字符转为大写
...
class WxPayResults extends WxPayDataBase
{
	 ...
	 * 检测签名
	 */
	public function CheckSign($WxCfg)
	{
		//fix异常
		if(!$this->IsSignSet()){
     ...
     * @throws WxPayException
     */
	public static function Init($xml,$WxCfg)
	{	
		$obj = new self();
		$obj->FromXml($xml);
		//fix bug 2015-06-29
